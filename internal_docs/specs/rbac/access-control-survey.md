...
- **Listing drives the architecture.** Point checks are easy to centralize; list queries
  decide whether the model can be enforced before pagination and at scale.
- **Stored edges are the common baseline in this survey.** All four products store coarse
  access as memberships, scoped permissions, or ACL rows that can be traversed by the
  database.
- **Attribute predicates appear only as a narrow refinement.** The predicate-based peer
  does not authorize from arbitrary resource content or telemetry; it gates on curated
  server-managed tags layered below workspace RBAC.
...
- **Allow-only remains the safer default** — *not* "never deny." Contractor walls,
  regulatory segregation, and break-glass are **legitimate** deny use cases; the point is that
  explicit deny changes query shape, explainability, and audit semantics enough that it must be
  **designed deliberately** (§11), not added as a column and discovered later.
...
- **Group** — a reusable *set of subjects*, so access is not granted one identity at a
  time. It is the **mirror of a role** (Layer 2 / §3): the same **junction vertex** shape,
  but its *incoming* edges are its members and its *outgoing* edges are the grants it holds.
  A role bundles the *permission* side of the edge; a group bundles the *subject* side —
  same machinery, opposite ends.
...
- **Role** — at bottom, a **named junction vertex** in the access graph (§3): its *incoming*
  edges say who holds it, its *outgoing* edges say what it grants, so "users with this role"
  and "permissions in this role" are just the two projections of one vertex. Products
  *usually* attach a permission bundle to it — hence the conventional shorthand *"a role is a
  reusable bundle of permissions"* — but that bundle is a **product choice, not the essence**
  (§3, "one primitive, four knobs"). A role and a group are the *same* junction; only the
...
- **Binding strength** — *who can change or shed this fact, and how fast do changes take
  ...
  binding strength ranks carriers by **who can change a fact and how fast** (authority-to-mutate
  / change-velocity) — **not** by tamper-resistance under breach. They diverge: a code constant
  resists *casual* change but a bad deploy widens it; a DB row changes *legitimately* with ease
  but is also what a compromise rewrites. Rank for the threat you actually mean.
...
check as a **bounded walk**. (They separate *relation tuples* from *permission computation* —
`check` vs. `expand`, with caching and consistency machinery — so "store every permission as an
edge" is the *shape*, not the literal implementation. ReBAC as a family is out of scope here;
this doc borrows only its mental model.)
...
| Role | a junction bundling permissions for a resource (e.g. `editor`). *Caveat:* a **global** tier (`admin`/`member`/`viewer`) used as a clearance is **evaluation context, not a graph vertex** — see "What is *in* the graph" below. |
...
The `which` axis is the unifier: a **group** is a `which` on the *subject* end (a predicate
over users — "is a member"), and a **tag rule** is a `which` on the *object* end (a predicate
over resources — "is tagged `env=prod`"). They are **the same mechanism on opposite ends of
the same edge** — which is exactly why Family B (below) is not a different paradigm, just
"move the predicate from the subject end to the object end." A point check evaluates one
edge; a **list query** can require evaluating or joining the predicate against candidate
objects — cheap when it maps cleanly to an **indexed, server-managed attribute** (a curated
tag column), and expensive when it must compose multiple conditions or run over
non-indexable/computed content. That query-time cost (and the carrier discipline it demands)
is why object-side predicates are the harder case.
...
A *custom global role* (a user-defined capability bundle, one per user) and *a group holding
a wildcard grant* are **the same graph machinery** with the knobs set differently — which is
why the two collapse under scrutiny, and why "is a role different from a group?" has no clean
yes/no *at the model level*. They still differ in practice — and the knobs are exactly where
those differences live: **cardinality** (one-per-user vs. many), **where capability lives**
(packaged vs. externally granted), and, downstream of those, **lifecycle, revocation, audit,
and UI semantics**. So: reason about the knobs, not the labels — and note that the last knob,
the operator, is the one that carries security weight.

**The principle, stated for reuse.** A role and a group are **one primitive — a named set**
(a junction vertex). *Extensionally* both classify users (a role's holders, a group's members),
which is why they feel interchangeable; but their **defining payload is dual**: a *role* is a
(nestable) **set of permissions**, a *group* is a (nestable) **set of users** — same junction
shape, dual element-types, bound by a grant edge `(subjects) ─[capability]→ object`. A product
exposing "roles" and "groups" with **parallel `member_*` fields** (one nesting over permissions,
one over users) is that duality in the schema. What makes one *behave* like a role or a group is
**product policy** — knobs (membership source, cardinality, scope, where capability lives,
composition operator). **Don't infer these from the words "role"/"group" — read the product's
rules.** Of the knobs, only the **operator** (floor `∪` / cap `∩` / deny `∖`) changes *what is
safe*; the rest are ergonomics, provisioning, and UI.
...
**Corollary — only subtraction conflicts.** This is the deepest consequence, and it reframes
the usual "how do all these sources compose?" worry. **Additive sources never disagree.** `∪`
is order-independent and monotone, so *any* number of grant sources — roles, groups, direct
grants, permission sets, tags — compose without conflict: the result is just their union, and
adding one more can never contradict the others. The *only* thing that can disagree with a
grant is a **subtractive** operator: a `∩` cap or a `∖` deny. So the count of **collection
mechanisms** is irrelevant to conflict — a system with ten ways to *grant* and no cap has no
composition conflicts at all; a system with one grant path and one cap does. **Conflict tracks
the number of subtractive operators, not the number of sources.** Practically: if two
authority sources ever "fight" over a subject, one of them is a cap or a deny — find *it*, not
the grants. (This is why an all-`∪` model is conflict-free by construction, and why every
"which source wins?" design question is really a question about a cap or a deny.)
...
- **Subject-level tiers / caps.** A global role (`ADMIN`/`VIEWER`) is a clearance **stamped on
  ...
  outgoing grant edges. Modeling a cap as an edge is exactly the deny-vs-cap error of §11.
...
A short FAQ on what this model exists to settle. The throughline: **don't reason from the words
"role" and "group" — read the knobs.**

- **Is a role a set of *users* or of *permissions*?** Both, at different levels. *Extensionally*
  a role is its holders (users); by **defining payload** it is a set of permissions — and the
  payload is the part you *author*. A group is the mirror (a set of users by payload).
- **How is a role different from a group?** Not intrinsically — both are named sets (§3, "one
  primitive"). The difference is **product policy**: cardinality, membership source, scope,
  whether capability is packaged into the set, and whether it composes as a **floor** or a
  **cap**.
- **Can a group have permissions attached directly?** In principle yes — but a user-set *with* a
  packaged capability bundle **is a role** (the role/group line is precisely *"is capability
  packaged in?"*). The products surveyed here keep them **separate**: a group is a bare subject
  set whose authority comes from grants/policies that *target* it.
- **Can a role be a bare subject set with *no* intrinsic permissions?** Yes — when capability is
  **external**. A policy-based layer can target roles by id while the **policy** carries the
  permission/effect/condition; there the role is just the targetable subject set. (Watch the
  layer: the *same* product may *also* attach RBAC permissions to that role elsewhere.)
- **Is *deny* the same as a *cap*?** No. A **cap** is subject/credential-local, knowable before
  any object is consulted (a `min` on a clearance). A **deny** is a negative rule/edge, often
  object-specific, found *during* resolution — it breaks short-circuit and monotonicity (§11).
  Deny can *emulate* a cap, at deny's cost.
- **Are credentials part of the access graph?** Usually no — a credential **wields** an identity
  and may **attenuate** it (§2). If a credential holds its own grants and lifecycle, it has
  become a first-class identity / service account.
- **What is a carrier, and why care?** Where an authorization fact physically lives — DB row,
  signed claim, header, tag, cache (§13). The *same logical edge* is safe or unsafe depending on
  whether its carrier is tamper-resistant and fresh.
- **The one question to ask of any product?** *What is the named subject set, where does
  capability live, what scope does it apply to, and does it compose as floor, cap, or deny?* The
  rest is the §6 grid.
...
teams sometimes reach for it. Treat the A/B split below as the axis *within* stored-vs-computed,
...
The set of vertices reached **is** the SQL `WHERE id IN (...)`. This family is cheap
*relative to Family B*: traversal over indexed foreign keys is what databases are built
for.

`id IN (...)` is the *conceptual* shape, not necessarily the literal SQL. With large
grant sets, downward inheritance, and pagination, real implementations express it as a
join, a semi-join (`EXISTS`), or a recursive CTE rather than an inlined id list, and the
reachable set can be large enough that materializing it needs care. The claim is not
that the query is trivial — a resolver still handles inheritance, group expansion,
subject unions, attenuation, and pagination — only that the work is a *traversal a
database is built to plan*, not a bespoke rule-to-SQL compilation.
...
**In practice, Family B is far narrower than its theory.** The one surveyed peer that
ships predicates restricts them to one kind of curated, server-managed resource tag, and
the rule language does not reference resource content or telemetry. A predicate like
"only spans where `cost > $5`" is simply unexpressible. That is the carrier discipline of
Section 13 enforced **by construction** rather than by convention: the security-bearing
attribute can only ever be privileged and admin-curated, never client-supplied or
content-derived. The same peer also closes the window between *creating* a resource and
*tagging* it — where it would briefly carry the wrong tags, and so the wrong access — with
a permissioned tag-at-creation path, so the gating tag is applied atomically at birth
rather than in a follow-up write. The lesson for anyone considering Family B: the
expressive power the theory advertises ("predicate over anything") is *not* what this
surveyed implementation ships; it ships "predicate over one curated attribute, applied at
birth," precisely because the alternative makes the resource's own content part of its
security surface.
...
- **The oracle is the seam.** As long as *every* check goes through it — no hand-written
  `WHERE id IN (...)` in resolvers, no `if is_admin: skip` shortcuts — the
  implementation behind it can evolve from "roles only" to "memberships" to "ACLs"
...
The point: Alice's *identity* is allowed to read those spans, but the *credential* she
is using cannot — the request is denied by the right-hand term of the formula, not the
left. Swap the ingest-only key for her browser session (full attenuation) and the
identical request is allowed. Two subjects, one identity, different answers — which is
exactly why the credential/identity split in the vocabulary is not pedantic.
...
The profiles below answer roughly #1–#5 for each vendor; §11–§14 cover #6–#10. A worked
instance: a peer whose "groups" come only from an IdP and *map to roles* (Q1–Q2) with capability
in the roles (Q3), composing additively with a deny-policy layer (Q5), is fully described by the
grid — the word "group" told you nothing; Q2 and Q3 told you everything.
...
identity-provider groups (or SSO attributes) map to a **role assignment within a
tenant** — "members of group X get role R in workspace W." The group is a transient
input evaluated at login; what persists is the *resolved role assignment* — often
written to a row, or in lighter integrations simply carried in session/token state and
re-derived from the IdP claims each session — not the group or its membership graph
(Vendor D and enterprise SSO integrations generally). SCIM-backed systems are the
boundary case: they may persist a membership graph precisely so deprovisioning can act
on it. The common property is **data minimization** — the descriptive group is not
retained as a grant subject, only the outcome it resolves to.
...
The case for service accounts is mostly about lifecycle and blast radius:

- **They outlive people.** A personal key dies when its owner is deprovisioned, so a
  pipeline authenticated by a departed employee's key breaks on offboarding. A service
  account is not tied to a person, so removing a human does not take down a workload.
- **Least privilege without a human's authority.** A personal key, unattenuated, carries
  all of its owner's power; a leaked admin's key is an admin key. A service account is
  granted only what the workload needs, bounding the worst case by design rather than by
  who minted the key.
- **Honest audit and ownership.** Grants, audit entries, and "who has access" dialogs
  name the workload, not a person standing in for a cron job.
- **Clean rotation.** Rotating or revoking a service account's credential touches one
  non-human identity and never entangles a person's interactive sessions.
...
"Viewer" changes everyone's access at once, and the change is invisible in a
grants-table audit log unless the audit unit is *effective-access change*, not *row
written*.
...
- **A group or role definition is edited.** Adding someone to a group, or redefining a
  role, changes access broadly and instantly — and a grants-table audit log shows
  nothing. The audit unit must be *effective-access change*.
...
fact on a weak carrier. Ordered weakest to strongest:
...
- **Tag** — editable by whoever edits the resource. Fine for descriptive metadata;
  dangerous the moment it *gates access* (see Section 11). If a tag must bear security
  weight, make it a privileged key or match on an immutable property.
...
One carrier becomes an **access path** of its own and deserves separate naming: the
**capability URL** — an unguessable share token embedded in a link. It collapses carrier
and grant into one: *possession of the link is the access.* It sits outside all four
families — no subject, no grant row, no predicate, just "whoever holds this may read,"
gated only by who is permitted to mint it. (At least one surveyed peer offers it, gated by a
dedicated `share` permission.) It is genuinely useful — frictionless external sharing — and
dangerous in a way grants are not: a leaked or forwarded link is a **silent grant** that no
"who can see X?" review will surface (the holder is no subject) and that persists until the
token is rotated, not until a membership changes. If offered, treat minting as its own
privileged action, make the tokens first-class **revocable** objects rather than opaque
URLs, and remember that they are a second door into the same data the oracle guards — one
...
**The single oracle is the natural emission point.** The same discipline that makes the
oracle the one seam for *deciding* access (Section 5) makes it the one place to *emit* an
audit record: every check already flows through it, so a decision and its log entry are
produced together, and there is no second code path that grants access without recording
it. A system with audit bolted on *after* the decision — at scattered call sites — will
log inconsistently for exactly the requests that matter. Designing the oracle to return
(or emit) the *reason* for a decision, not just the boolean, is also what later makes deny
explainable ("why can't I see this?", Section 17) and makes the change log able to state
consequences rather than mutations.

What to record is the conventional tuple — **who** (subject *and* the credential, since
Section 2 splits them), **what** action, **on what** resource, **when**, **from where**,
and the **result** (allow/deny, and on deny, why). Two properties matter more than the
fields: the log must be **append-only and tamper-evident** (an actor must not be able to
edit away their own entries — a binding-strength question in the sense of Section 13), and
its **retention** is a policy input, not an afterthought. None of this is access *control*;
all of it is what makes access control *accountable*, which is why a survey that stops at
the decision is only half the picture.
...
| SCIM provisioning | varies | varies | SSO (no SCIM observed) | yes (enterprise) |

---

## 16. Cross-Cutting Findings

1. **Enumerated-grant is the mainstream choice in this surveyed set.** All four store
...
4. **Roles trend toward data, with custom roles increasingly common.** Three of the four
   model roles as data rather than code; of those, two support fully custom roles and
   one a partial set — frequently behind an enterprise tier.
...
- **Let the shape of the requirement pick the family.** If the demand is "these teams
  see these resources" and "give this person this resource," the enumerated-grant family
  answers it with one row per grant and a cheap list filter, and is the dominant pattern
  in this survey. The attribute-predicate family is worth its query-time cost only when a
  real, named requirement is shaped "everything matching X" — and it brings a
  tag-as-security-surface problem that requires distinguishing security tags from
  descriptive ones *before* any tag gates access.

- **Allow-only is a safe default**, with explicit deny deferred until a concrete need
  appears. Be precise about what is cheap to reserve and what is not. If deny is
  plausible, an `effect` column constrained to `allow` for now avoids a later migration;
  if deny is genuinely out of scope, omitting the column avoids implying semantics that
  do not exist. The *semantics* of deny are not cheap: precedence rules (does a deny
  beat an allow at the same specificity, or only a broader one?), audit and
  explainability ("why can't I see this?"), the changed shape of every list query (a
  deny is a `NOT IN` carved out of an allow set), and the UX of authoring and reasoning
  about conflicting rules. Either way, do not write a single `deny` row until those
  semantics are designed. The corollary is to route every check through a single oracle
  now, so adding deny later is one contained change rather than an invalidation of every
  call site that learned the allow-only shortcut.

- **Decide the unit of isolation consciously.** Pure object-grain grants deliver fine
  control without a tenant layer (names may collide); workspace-grain delivers name
  isolation and per-team admins but requires that layer up front. They are not mutually
  exclusive — an object-grain model can add an org/tenant object type with downward
  inheritance (as one surveyed peer does), and a workspace-grain model can add
  per-object ACLs or tag refinement below it (Section 4 hybrid).

- **Choose group handling with eyes open to the data trade.** Treating groups as
  first-class grant subjects enables per-object, team-level access and service-account
  membership, but persists the membership graph. The role-mapping alternative stores
  only the resolved assignment (better minimization) but cannot grant a group access to
  a specific resource. The right answer follows from the isolation-unit choice.

- **Make the change-time decisions up front** (Section 12): new-resource default
  visibility per type, a revocation budget, unauthorized-read-≡-not-found, and a subject
  union type (`user | service_account | group`) in the first schema that has subjects —
  even if only users are populated initially. Each costs nothing now and a rewrite
  later.

- **Treat provisioning as a maturity axis, not a correctness one.** Login-time SSO
  mapping is sufficient to ship; continuous SCIM provisioning — and the prompt
  deprovisioning it enables — is the enterprise-grade upgrade.
...
constrains the next, and most cannot be changed cheaply once data exists. The list is
deliberately product-agnostic — substitute your own resource and container names where
appropriate.
