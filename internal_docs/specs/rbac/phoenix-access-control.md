...
- **For trace-bearing, project-contained data there are only two access *worlds*, not many
  kinds of resource.** The **telemetry world** (machine-created projects, ownerless →
  admin-only) and the **dataset world** (human-created datasets/prompts → creator-owned) are
  the only two access *roots*. Everything else — experiment traces, evaluator traces,
  online-eval scores — is a **run-artifact rooted at the data context it evaluates** (a dataset
  offline, a production project online), not a third kind of project. "Project" is just the
  span store, reused (§2.8). This collapses a pile of special cases into one principle: *root
  each thing at its real data context.* (Deployment-global objects that hang off **no** project
  — secrets, the shared evaluator/model/label library, system config — sit outside both worlds
  and get their own treatment in §6.)
...
**The cost of ungated writes: unbounded invisible projects.** This is not a *security*
problem (ordinary users can't see admin-only projects), but it is an **operational** one: any
key can spawn arbitrarily many admin-only projects — clutter in the admin's all-projects
view, triage load (admins must sort real from junk before granting), and a soft
resource-DoS. It is the deliberate flip side of leaving writes open (the same choice that
dissolves the §2.10 ingestion-breakage and the bypass above). If it bites, the mitigations
are a per-key **creation rate-limit / quota** or, eventually, gating writes — neither is in
the strawman. The takeaway: ungated writes are *read-safe* but not automatically acceptable
*product* behavior, and "unbounded invisible project creation" is the cost to weigh.
...
#### 2.8 Two access worlds: telemetry projects and the dataset-rooted eval world

Step back from "kinds of project" to the structure underneath. For trace-bearing,
project-contained data there are really **two access worlds**, with only two access *roots*
(deployment-global objects outside any project are a separate matter — §6):
...
Everything else hangs off one of these. The clarifying question is *what produces a trace?*
...
So the rule is one principle, not a taxonomy: **eval traces (and the scores on them) are
...
**Playground is neither world — it's per-user scratch.** Its prompts are the most sensitive
...
Worth flagging that this is a **third pattern**, not a clean fit to either world: playground
traces are *machine-ingested* (telemetry-world mechanics) yet *creator-private*
(dataset-world semantics). Implementers should treat it as a deliberate hybrid — per-user
...
#### 2.9 Moving data across boundaries (trace transfer)

Trace transfer is the concrete instance of a general rule worth stating first, because it
...
This is the standard polymorphic-vs-per-type trade. The prototype takes the generic table
...
- **3.11 Point-in-time audit needs effective-access history, if compliance requires it.**
  The survey's change-log unit is *effective-access change*, but support and compliance
  often ask "who could see X **at time T**?", not just now — and the answer drifts as
  groups, roles, and grants all change over time. Answering it requires
  either periodic **effective-access snapshots** or enough **event history** to reconstruct
  effective access at an arbitrary past instant. This carries real storage/retention cost,
  so treat it as conditional: decide up front whether point-in-time answers are a
  requirement, because retrofitting the history after the fact is impossible — the events
  that were not recorded are gone.
