...
export function SettingsRolesPage() {
  ...
  const [commitCreate, isCreating] =
    useMutation<SettingsRolesPageCreateMutation>(graphql`
      mutation SettingsRolesPageCreateMutation($input: CreatePermissionSetInput!) {
  ...
  const [commitPatch, isPatching] =
    useMutation<SettingsRolesPagePatchMutation>(graphql`
      mutation SettingsRolesPagePatchMutation($input: PatchPermissionSetInput!) {
  ...
  const [commitDelete, isDeleting] =
    useMutation<SettingsRolesPageDeleteMutation>(graphql`
      mutation SettingsRolesPageDeleteMutation($input: DeletePermissionSetInput!) {
  ...
  const handleSave = useCallback(() => {
    ...
    const onCompleted = () => {
      notifySuccess({
        title: editingId == null ? "Role created" : "Role updated",
  ...
  return (
    <Flex direction="column" gap="size-200" width="100%">
      ...
      <Card
        title={
          editingId == null ? "Create resource role" : "Edit resource role"
        }
        subTitle="Reusable permission presets for project, dataset, and prompt grants."
      ...
      <Card
        ...
        subTitle="Built-in roles are read-only. Custom roles can be edited or deleted."
        titleSeparator={false}
      >
        <table css={tableCSS}>
          <thead>
                    ...
                    <Flex justifyContent="end" gap="size-100">
                      {!role.isBuiltIn && (
                        <>
                          ...
                          <ConfirmButton
                            buttonText="Delete"
                            buttonAriaLabel={`Delete role ${role.name}`}
                            isDisabled={isMutating}
                            title="Delete role"
                            message={
                              <>
                                Delete the role <strong>{role.name}</strong>?
                                Existing grants that use it will fall back to
                                view-only access.
                              </>
                            }
