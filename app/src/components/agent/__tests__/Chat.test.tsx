...
function renderChatView(
  root: Root,
  {
    ...
    error,
    status = "ready",
    initialPendingInputBySessionId,
    sendMessage = vi.fn(),
    retryMessage,
    rewindToMessage,
    forkFromMessage,
    ...
    error?: Error;
    status?: "ready" | "error";
    initialPendingInputBySessionId?: Record<string, string>;
    ...
    root.render(
      <ThemeProvider themeMode="light" disableBodyTheme>
        <AgentProvider
          ...
          {...(initialPendingInputBySessionId
            ? { pendingInputBySessionId: initialPendingInputBySessionId }
            : {})}
        >
          <ChatView
...
describe("ChatView", () => {
  ...
  it("restores pending input when switching to a keyed session", () => {
    renderChatView(root, {
      sessionId: "session-1",
      initialPendingInputBySessionId: { "fork-session": "edit me" },
    });
